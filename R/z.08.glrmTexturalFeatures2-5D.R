...
glrlmTexturalFeatures25D <- function(imgObj,n_grey){
  ...
  for (i in 1:dim(imgObj)[3]){
    ...
    Nv[i] <- length(imgObj[,,i][which(!is.na(imgObj[,,i]))])
  }
  ...
  featNames <- c("F_rlm_2.5D.sre","F_rlm_2.5D.lre","F_rlm_2.5D.lgre","F_rlm_2.5D.hgre","F_rlm_2.5D.srlge",
                 "F_rlm_2.5D.srhge","F_rlm_2.5D.lrlge","F_rlm_2.5D.lrhge","F_rlm_2.5D.glnu","F_rlm_2.5D.glnu.norm","F_rlm_2.5D.rlnu","F_rlm_2.5D.rlnu.norm", "F_rlm_2.5D.r.perc",
                 "F_rlm_2.5D.gl.var","F_rlm_2.5D.rl.var","F_rlm_2.5D.rl.entr")
  ...
  for (iter in seq_len(length(sumtot))){
    ...
    #Calculate features
    #Short runs emphasis
    F_rlm$F_rlm_2.5D.sre[iter]         <- (1/Ns) * sum(df.r_j$r_j/(df.r_j$j^2))
    
    #Long runs emphasis
    F_rlm$F_rlm_2.5D.lre[iter]         <- (1/Ns) * sum(df.r_j$r_j*(df.r_j$j^2))
    
    #Low grey level run emphasis
    F_rlm$F_rlm_2.5D.lgre[iter]         <- (1/Ns) * sum(df.r_i$r_i/(df.r_i$i^2))
    
    #High grey level run emphasis
    F_rlm$F_rlm_2.5D.hgre[iter]         <- (1/Ns) * sum(df.r_i$r_i*(df.r_i$i^2))
    
    #Short run low grey level emphasis
    F_rlm$F_rlm_2.5D.srlge[iter] <- (1/Ns) * sum(df.R$n/((df.R$i^2)*(df.R$j^2)))
    
    #Short run high grey level emphasis
    F_rlm$F_rlm_2.5D.srhge[iter] <- (1/Ns) * sum((df.R$n)*(df.R$i^2)/(df.R$j^2))
    
    #Long run low grey level emphasis
    F_rlm$F_rlm_2.5D.lrlrlm_25D_merged.dfge[iter] <- (1/Ns) * sum((df.R$n)*(df.R$j^2)/(df.R$i^2))
    
    #Long run high grey level emphasis
    F_rlm$F_rlm_2.5D.lrhge[iter] <- (1/Ns) * sum((df.R$n)*(df.R$j^2)*(df.R$i^2))
    
    #Grey level non-uniformity
    F_rlm$F_rlm_2.5D.glnu[iter] <- (1/Ns) * sum(df.r_i$r_i^2)
    
    #Grey level non-uniformity normalized
    F_rlm$F_rlm_2.5D.glnu.norm[iter] <- (1/Ns^2) * sum(df.r_i$r_i^2)
    
    #Run length non-uniformity
    F_rlm$F_rlm_2.5D.rlnu[iter] <- (1/Ns) * sum(df.r_j$r_j^2)
    
    #Run length non-uniformity normalized
    F_rlm$F_rlm_2.5D.rlnu.norm[iter] <- (1/Ns^2) * sum(df.r_j$r_j^2)
    
    #Run percentage
    
    F_rlm$F_rlm_2.5D.r.perc[iter] <- Ns/sum(Nv)
    
    #Grey level variance
    p_ij <- df.R$n/sum(df.R$n)
    mu_i <- sum(df.R$i * p_ij)
    F_rlm$F_rlm_2.5D.gl.var[iter] <- sum((df.R$i - mu_i)^2 * p_ij)
    
    #Run length variance
    mu_j <- sum(df.R$j * p_ij)
    F_rlm$F_rlm_2.5D.rl.var[iter] <- sum((df.R$j - mu_j)^2 * p_ij)
    
    #Run entropy
    F_rlm$F_rlm_2.5D.rl.entr[iter] <- - sum(p_ij * log2(p_ij))
  }
